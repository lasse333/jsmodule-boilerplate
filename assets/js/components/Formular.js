import { addChildren, clearChildren, createElement, changeStyle } from "../FastHTML.js";
import RichTextAreaStyleSheet from "../../css/richTextArea.css" assert {type:"css"}
...
export default function Formular(inputs) {
    ...
    for (let inputName in inputs) {
        ...
        if (type == "textarea") element = createElement(type, attributes)
            ...
            form.richTextInputs.push(new RichTextArea(element,
                {
                    attributes
                }    
            ))
        ...
        else if (type == "submit") element = createElement("input", inputs[inputName])
        else if (type == "label") element = createElement(type, Object.assign(attributes, {innerText: name}))
        else element = createElement("input", Object.assign(inputs[inputName], {name}))
...
class RichTextArea{
    constructor(element, options) {
        ...
        changeStyle(this.element, RichTextAreaStyleSheet)
        this.value = this.options.attributes.value

        this.toolbar = createElement("div", {class: "tool-bar"}, [
            createElement("button", {onclick: ()=> { this.setTextStyle({fontWeight: "bold"})}, innerText: "Make Bold"}),
            createElement("button", {onclick: ()=> { this.setTextStyle({color: "#c0392b"}) }, innerText: "Make red"}),
            createElement("button", {onclick: ()=> { this.clearStyle() }, innerText: "Clear Formatting"}),
            createElement("button", {onclick: ()=> { this.insertList("ul") }, innerText: "Insert Unordered List"})
        ])

        this.richText = createElement("div", {class: "rich-text"}, [this.value ?? ""])


        this.textArea = createElement("textarea", Object.assign(this.options.attributes, {value: this.value, class: "raw-text"}))
    ...
    setTextStyle = (style) => {
        ...
        if (
            ...
            &&
            (this.selection.startContainer.parentElement.childNodes.length - this.selection.startContainer.parentElement.children.length) === 1
        ) {
            ...
            newSelection.setBaseAndExtent(wrapper, 0, wrapper, wrapper.childNodes.length)
            this.selectEvent()
        }
    ...
    clearStyle = () => {
        if (
            ...
            if (this.selection.startContainer.parentElement.tagName === "SPAN") {
                this.selection.startContainer.parentElement.outerHTML = this.selection.startContainer.parentElement.innerHTML
            }

        } else {
            
            let text = this.element.shadowRoot.getSelection().toString().replaceAll("\n", "<br>")
